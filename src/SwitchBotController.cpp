...
// 静的メンバーの定義
const NimBLEUUID SwitchBotController::serviceUUID(
    "cba20d00-224d-11e6-9fb8-0002a5d5c51b");
const NimBLEUUID SwitchBotController::charUUID(
...
SwitchBotController::SwitchBotController()
  ...
  logger.info("SwitchBotController created");
}
...
bool SwitchBotController::connectAndSendCommand(const char* deviceAddress,
                                                uint8_t* command,
  ...
  NimBLEAddress targetAddress =
      NimBLEAddress(std::string(deviceAddress), BLE_ADDR_RANDOM);
  ...
  logger.info("Created BLE client");
  ...
  try {
    ...
    logger.info("Connected to SwitchBot!");
    ...
    if (pRemoteCharacteristic == nullptr) {
      logger.error("Failed to find characteristic");
      pClient->disconnect();
      NimBLEDevice::deleteClient(pClient);
      pClient = nullptr;
      deviceConnected = false;
    ...
    logger.error("Exception occurred: " + String(e.what()));
...
bool SwitchBotController::press(const char* deviceAddress) {
  ...
  // 直接接続してコマンド送信
  return connectAndSendCommand(
      deviceAddress, const_cast<uint8_t*>(pressCommand), pressCommandLength);
}

bool SwitchBotController::isConnected() const { return deviceConnected; }
